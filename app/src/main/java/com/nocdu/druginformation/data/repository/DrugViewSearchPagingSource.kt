 ...
 *  서버 API는 0부터 페이지 시작
 */
class DrugViewSearchPagingSource(
    private val shape: String? = null,
    private val dosageForm: String? = null,
    private val printFront: String? = null,
    private val printBack: String? = null,
    private val colorClass: String? = null,
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Document> {
        return try {
            ...
            val response = RetrofitInstance.api.searchViewDrugs(
                shape = shape?.ifEmpty { null },
                dosageForm = dosageForm?.ifEmpty { null },
                printFront = printFront?.ifEmpty { null },
                printBack = printBack?.ifEmpty { null },
                colorClass = colorClass?.ifEmpty { null },
                line = line?.ifEmpty { null },
                page = pageNumber,
                size = PAGING_SIZE
            )
